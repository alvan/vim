...
" 
" Version 1.1.0 by Alvan
" -------------
"  * Fixed some bug
"
" Version 1.1.1 by Alvan
" -------------
"  * Add pdv_cfg_Commente = " *" to prevent from adding blank char at the end

" Exit if already loaded
if exists("g:loaded_php_doc")
    finish
endif
let g:loaded_php_doc = "Version 1.1.1"
...
let g:pdv_re_attribute = '^\s*\(\(private\s\|public\s\|protected\s\|var\s\|static\s\)\+\)\s*\$\([^ ;=]\+\)[ =]*\(.*\);\?$'

" [:space:]*(final|abstract)[:space:]*(class|interface)+[:space:]+\(extends ([:identifier:])\)?[:space:]*\(implements ([:identifier:][, ]*)+\)?
...
func! PhpDocFunc()
    ...
    exe l:txtBOL . g:pdv_cfg_Commente . g:pdv_cfg_EOL
