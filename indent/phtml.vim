...
" Last Change:	2011-09-26
" Newsletter:	http://www.2072productions.com/?to=php-indent-for-vim-newsletter.php
" Version:	1.34-1
"
" Changes: 1.34-1	- Fix: string start with <script> and end with </script>
"                         in current line would be treated as html code
"                         (By Alvan)
"
...
let s:notPhpHereDoc = '\%(break\|return\|continue\|exit\|else\)'
    let s:blockstart = '\%(\%(\%(}\s*\)\=else\%(\s\+\)\=\)\=if\>\|else\>\|while\>\|switch\>\|case\>\|default\>\|for\%(each\)\=\>\|declare\>\|class\>\|interface\>\|abstract\>\|try\>\|catch\>\)'
        ...
        function! GetPhpIndent()
            ...
            if b:InPHPcode
                " Add by Alvan
                if cline =~? '</script>\s*$' && cline =~? '^\s*<script\(>\|\s\)'
                    let b:InPHPcode = 0
                    let b:InPHPcode_tofind = s:PHP_startindenttag

                    " Was last line containing a PHP end tag ?
                elseif !b:InPHPcode_and_script && last_line =~ '\%(<?.*\)\@<!?>\%(.*<?\)\@!' && IslinePHP(lnum, '?>')=~"Delimiter"
